...
function rotate(array, steps){
	//checking if number is positive so will rotate to the right
	if(steps > 0){
		for(var i=0; i< steps; i++){
			//remove last element and attach it to the begining of array
			array.unshift(array.pop());
		}
	//checking if number is negative so will rotate to the left
	} else if(steps < 0) {
		for(var i=0; i< Math.abs(steps); i++){
			//remove first element and attach it to the end of array
			array.push(array.shift());
		}
	}
	//if its 0 it will do nothing just return the same array
	return array;
}
